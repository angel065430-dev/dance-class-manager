...
export function useAuth() {
   ...
   * 前端完全不自行寫入這兩張表。
   */
  async function signUpStudent(rawPhone: string, pin: string, rawName: string, rawLineName: string | null = null) {
    ...
    const { data: functionData, error: functionError } = await supabase.functions.invoke(
      ...
      { body: { phone: phoneResult.e164, pin, name, line_display_name: lineDisplayName } },
    )
    if (functionError)
