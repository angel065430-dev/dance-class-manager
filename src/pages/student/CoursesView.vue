...
import { submitFullTermRegistrations, generateIdempotencyKey } from '@/services/registrations'
import type { ClassWithAvailability, SubmitRegistrationsResult } from '@/types/database'
...
function toggleSelect(classId: string) {
  ...
  // 觸發響應性更新（Set 的內部變更不會自動被 Vue 偵測到）。
...
async function handleSubmit() {
  ...
  try {
    ...
    loadError.value = err instanceof Error ? err.message : '報名送出失敗，請稍後再試'
  } finally {
...
<template>
  <section class="mx-auto max-w-2xl">
    ...
    <ul v-if="!loading && classes.length > 0" class="mt-4 space-y-3">
      <li
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500">{{ cls.venue_name }} ・ {{ cls.term_name }}</p>
          <p class="text-sm text-gray-500">整期價格 NT$ {{ cls.full_term_price }}</p>
          <p class="text-sm" :class="cls.remaining_seats === 0 ? 'text-red-600' : 'text-gray-500'">
            剩餘名額：{{ cls.remaining_seats ?? '—' }}（僅供參考）
          </p>
        </div>

        <label v-if="isLoggedIn && !isAdmin" class="flex items-center gap-2">
          <input
            type="checkbox"
    ...
    <p v-if="!loading && classes.length === 0 && !loadError" class="mt-4 text-sm text-gray-500">
      目前沒有開放報名的期課。
    </p>

    <div v-if="isLoggedIn && !isAdmin && classes.length > 0" class="mt-6">
      <button
        type="button"
        ...
        @click="handleSubmit"
      >
        {{ submitting ? '送出中...' : `送出報名（已選 ${selected.size} 堂）` }}
      </button>
    </div>
    ...
    >
      <template v-if="result.success">
        <p class="font-medium text-green-800">報名成功！</p>
        <ul class="mt-2 list-disc pl-5 text-sm text-green-800">
          <li v-for="c in result.classes" :key="c.registration_id">{{ c.class_name }}</li>
        </ul>
        <RouterLink
          to="/my-registrations"
          class="mt-3 inline-block text-sm text-green-800 underline"
          >查看我的報名</RouterLink
        >
      </template>
      <template v-else>
        ...
        <ul v-if="result.failures.length > 0" class="mt-2 list-disc pl-5 text-sm text-red-800">
          <li v-for="f in result.failures" :key="f.class_id">
            {{ f.class_name ?? f.class_id }}：{{ FAILURE_REASON_TEXT[f.reason] ?? f.reason }}
          </li>
        </ul>
        <p v-else class="mt-2 text-sm text-red-800">{{ loadError }}</p>
        <p class="mt-2 text-sm text-red-700">請調整勾選後重新送出。</p>
      </template>
    </div>
  </section>
</template>
